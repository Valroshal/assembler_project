...
void createObjectFile(char *name, int IC, int DC, int *memoryArr)
{
 ...
 /* Print IC and DC */
 fprintfBase10(file, IC, 1);
 fprintf(file, " ");
 fprintfBase10(file, DC, 1);

 /* Print all of memoryArr */
    for (i = 0; i < IC + DC; i++)
 {
  fprintf(file, "\n");
  fprintfBase64(file, memoryArr[i], 2);
 }
...
void createEntriesFile(char *name)
{
	...
	for (i = 0; i < g_entryLabelsNum; i++)
	{
		...
		fprintfBase10(file,locateLabelPointer(g_entryLines[i]->lineStr)->address, 1);
...
void createExternFile(char *name, lineInfo *linesArr, int linesFound)
{
	...
	for (i = 0; i < linesFound; i++)
	{
		...
		{
			label = locateLabelPointer(linesArr[i].op1.str);
			if (label && label->isExtern)
			{
				...
				fprintfBase10(file, linesArr[i].op1.address, 1);
				firstPrint = FALSE;
			}
		}
		...
		{
			label = locateLabelPointer(linesArr[i].op2.str);
			if (label && label->isExtern)
			{
				if (firstPrint)
				{
				...
				fprintf(file, "%s\t\t", label->name);
				fprintfBase10(file, linesArr[i].op2.address, 1);
				firstPrint = FALSE;
			}
		}
	}
