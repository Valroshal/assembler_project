...
labelInfo *addLabelToArr(labelInfo label, lineInfo *line)
{
	...
	if (!correctLabel(line->lineStr, line->lineNum, TRUE))
	{
		/* Illegal label name */
		line->isError = TRUE;
		return NULL;
...
char *findLabel(lineInfo *line, int IC)
{
	...
	*labelEnd = '\0';
	...
	if (!wordWithoutSpaces(line->lineStr))
	{
...
void parseDataDirc(lineInfo *line, int *IC, int *DC)
{
	...
	FOREVER
	{
		...
		operandTok = findFOperand(line->lineStr, &endOfOp, &foundComma);

		/* Add the param to g_dataArr */
		if (correctNum(operandTok, MEMORY_WORD_LENGTH, line->lineNum, &operandValue))
		{
...
void parseStringDirc(lineInfo *line, int *IC, int *DC)
{
	...
	cutEdgesStr(&line->lineStr);

	if (correctString(&line->lineStr, line->lineNum))
	{
...
bool areLegalOpTypes(const command *cmd, operandInfo op1, operandInfo op2, int lineNum)
{
	...
	if (op2.isRandom)
	{
		printError(lineNum, "Destination operand can't use the \"%s\" addressing method.", op2.str);
		return FALSE;
	}
     */
...
void parseOpInfo(operandInfo *operand, int lineNum)
{
	...
	if (*operand->str == '@')
	{
		...
		/* Check if ther is on white space after the "@" */
	...
	else if (correctLabel(operand->str, lineNum, FALSE))
	{
		operand->type = LABEL;
	}
    /* Check if the type is NUMBER */
     else if (correctNum(operand->str, MEMORY_WORD_LENGTH - 2, lineNum, &value))
    {
        operand->type = NUMBER ;
    }
    /* The type is INVALID */
...
void parseCmdOperands(lineInfo *line, int *IC, int *DC)
{
	...
	FOREVER
	{
		...
		if (spacesAndTabs(line->lineStr) || numOfOpsFound > 2)
		{
		...
		/* Parse the opernad*/
		line->op2.str = findFOperand(line->lineStr, &startOfNextPart, &foundComma);
...
{
	int cmdId = comID(line->commandStr);
...
void parseLine(lineInfo *line, char *lineStr, int lineNum, int *IC, int *DC)
{
	...
	line->commandStr = findFirstCh(line->lineStr, &startOfNextPart);
	line->lineStr = startOfNextPart;
